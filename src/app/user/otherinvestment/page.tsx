...
const PlanCard = ({ plan }: { plan: Plan }) => {
  ...
  const senderData = useSelector((state: any) => state.user);
  ...
  const createPackage = async () => {
    ...
    try {
      ...
      await axios.post(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/packages/giftpackage/${userId}`,
        {
      ...
      toast.success(`Investment Gift Successful 🎉 (${plan.name})`);
      ...
      toast.error(
        err?.response?.data?.message || "Error processing investment"
      );
    } finally {
      setLoading(false);
    }
      ...
      >
        ...
        <div className="p-6 pt-0 mt-auto">
          <Button className="w-full bg" onClick={() => setOpen(true)}>
            Gift {plan.name} <ChevronRight className="ml-2 h-4 w-4" />
          </Button>
        </div>
      </div>
        ...
        <DialogContent className="bg-slate-950 border-slate-800">
          ...
          <div className="space-y-4">
            <div className="bg-slate-900 p-3 rounded-lg border border-slate-800">
              ...
              <p className={`text-xl font-bold ${plan.color}`}>{plan.name}</p>
            </div>

            <div>
              <Label className="text-slate-300">Recipient UserId</Label>
              <Input
                type="text"
      ...
      <div className="max-w-7xl mx-auto px-6 py-16">
        <div className="text-center mb-16">
          <h1 className="text-5xl font-extrabold text-white">
            Gift Investment to Others
          </h1>
          <p className="text-slate-400 mt-4">
            Choose a secure high-yield investment plan for someone else
          </p>
        </div>
